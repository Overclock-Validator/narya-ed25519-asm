...
| Does either selector transpose preserve signature-to-lane identity? | partial | `NaryaFormal.TransposeX86.projective_assembled_shuffle_blocks_decode`, `affine_assembled_shuffle_blocks_decode`, `expected_block_semantics` | Every assembled eight-instruction shuffle block decodes to the proved exact 4×4 register transpose, independent of incoming scratch values. Source pointer setup, masked loads, stores, and whole-leaf ABI execution are still covered by source certificates/native tests rather than the Lean byte decoder. |
| Is the radix-`2^21` group-order fold/carry algebra correct, and why is the reconstructed result canonical? | partial | `NaryaFormal.ScalarReduction.fold_polynomial_exact`, `radix12_fold_mod_order`; `NaryaFormal.ScalarReduction.first_final_fold_value_abs_lt_radix12`, `second_fold_makes_canonical` | Exact fold/local-carry algebra plus the one-window canonical-tail theorem under source-certificate checkpoint bounds. Full 512-bit C parse, packing, and assembled-byte refinement remain open. |
