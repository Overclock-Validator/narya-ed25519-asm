...
| Do the five decoded output stores preserve lane identity and write the proved limbs to the correct rows? | closed | `NaryaFormal.X86.run_store_phase_refines` | Exact five-store suffix, explicit writable permissions, row isolation, read-back, register and permission preservation. A complete frame theorem for every byte outside the 320-byte output remains open. |
